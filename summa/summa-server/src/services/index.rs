...
impl Index {
    pub fn index_registry(&self) -> &IndexRegistry {
        &self.indexer.index_registry
    }

    pub async fn get_index_holder(&self, index_alias: &str) -> SummaServerResult<Handler<IndexHolder>> {
        Ok(self.indexer.index_registry.get_index_holder(index_alias).await?)
    }
    ...
    pub async fn new(server_config_holder: &Arc<dyn ConfigProxy<crate::configs::server::Config>>) -> SummaServerResult<Index> {
        ...
        let data_path = server_config_holder.read().await.get().data_path.clone();
        Ok(Index {
            server_config: server_config_holder.clone(),
            indexer: Indexer::new(core_config_holder, data_path),
    ...
    pub(crate) async fn setup_indices(&self) -> SummaServerResult<()> {
        ...
        *self.indexer.index_registry.index_holders().write().await = index_holders;

        for (consumer_name, consumer_config) in self.server_config.read().await.get().consumers.iter() {
            let index_holder = self.indexer.index_registry.get_index_holder(&consumer_config.index_name).await?;
    ...
    pub async fn attach_index(&self, attach_index_request: proto::AttachIndexRequest) -> SummaServerResult<Handler<IndexHolder>> {
        let index_path = self.indexer.data_path.join(&attach_index_request.index_name);
    ...
    pub async fn create_index(&self, create_index_request: proto::CreateIndexRequest) -> SummaServerResult<Handler<IndexHolder>> {
        ...
        let (index, index_engine_config) = match create_index_request.index_engine {
            None | Some(proto::create_index_request::IndexEngine::File(proto::CreateFileEngineRequest {})) => {
                let index_path = self.indexer.data_path.join(&create_index_request.index_name);
    ...
    pub async fn create_consumer(&self, create_consumer_request: proto::CreateConsumerRequest) -> SummaServerResult<String> {
        let index_holder = self.indexer.index_registry.get_index_holder(&create_consumer_request.index_name).await?;
    ...
    pub async fn stop(&mut self, force: bool) -> SummaServerResult<()> {
        ...
        if !force {
            ...
            for index_holder in self.indexer.index_registry.index_holders_cloned().await.values() {
                    ...
                    loop {
                        tokio::select! {
                            _ = tick_task.tick() => {
                                ...
                                let index_holders = index_service.indexer.index_registry.index_holders_cloned().await;
    ...
    pub async fn search(&self, search_request: proto::SearchRequest) -> SummaServerResult<Vec<proto::CollectorOutput>> {
        let index_holder = self.indexer.index_registry.get_index_holder(&search_request.index_alias).await?;
        ...
        let collector_outputs = index_holder
            ...
            .await?;
        Ok(self.indexer.index_registry.finalize_extraction(collector_outputs).await?)
    }
