...
### Retry: fail -> analyze -> rewrite prompt -> retry, max 3 then report
...
async function main() {
  ...
  const tm = await ask('Thinking model [zai/glm-5]: ') || 'zai/glm-5';
  const em = await ask('Execution model [zai/glm-4.7]: ') || 'zai/glm-4.7';
