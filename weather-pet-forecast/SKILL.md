...
```bash
# Fetch and analyze weather for a city (English output, default)
...
# Or with short flag
curl -s "wttr.in/Beijing?format=j1" | python3 scripts/weather_analysis.py "Beijing" -l zh
```

**Language Options:**
- `--lang en` or `-l en`: English output (default)
- `--lang zh` or `-l zh`: Chinese output
...
- Output language controlled by `--lang` parameter (en/zh)
